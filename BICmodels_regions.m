function BICmodels_regions(region)
...
if contains(region, 'mare') 
    indxRegion=find(craterData.mare);
elseif contains(region, 'spa') 
    indxRegion=find(craterData.insSPA);
end

newRBA=newRBA(indxRegion,:); %select rba data from region
